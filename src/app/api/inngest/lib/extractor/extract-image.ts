...
export async function extractImage(buffer: Buffer): Promise<ExtractedDocument> {
  ...
  const getCurrency = (): string | null => {
    const amount = invoice['InvoiceTotal'] ?? invoice['SubTotal'] ?? invoice['TotalTax']
    return amount?.kind === 'currency' ? ((amount.value as any)?.currencyCode ?? null) : null
  }
  ...
  if (itemsField?.kind === 'array') {
    for (const item of itemsField.values ?? []) {
      ...
      const getItemCurrencyValue = (key: string): number | null => {
        ...
        if (field?.kind === 'currency') return (field.value as any)?.amount ?? null
        if (field?.kind === 'number') return field.value as number
        return null
      }

      lineItems.push({
        description: fields['Description']?.kind === 'string'
  ...
  const subtotal = getNumber('SubTotal') ??
    (invoice['SubTotal']?.kind === 'currency' ? (invoice['SubTotal'].value as any)?.amount : null)

  const tax = getNumber('TotalTax') ??
    (invoice['TotalTax']?.kind === 'currency' ? (invoice['TotalTax'].value as any)?.amount : null)

  const total = getNumber('InvoiceTotal') ??
    (invoice['InvoiceTotal']?.kind === 'currency' ? (invoice['InvoiceTotal'].value as any)?.amount : null)
